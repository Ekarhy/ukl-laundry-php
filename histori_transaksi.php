...
<br></br>
  <div class="container">
    ...
    <tbody>
        <?php 
        include "koneksi.php";
        $qry_histori=mysqli_query($conn,"select *, transaksi.id as id_transaksi from transaksi join member on transaksi.id_member = member.id join user on transaksi.id=user.id order by transaksi.id desc");
        $no=0;
        while($dt_histori=mysqli_fetch_array($qry_histori)){
            ...
            $subtotal="<ol>";
            $qty="<ol>";
            $qry_paket=mysqli_query($conn,"select * from  detail_transaksi join paket on paket.id_paket=detail_transaksi.id_paket where id_transaksi = '".$dt_histori['id']."'");
            ...
            $qty.="</ol>";
            //menampilkan status
            $qry_cek_bayar=mysqli_query($conn,"select * from transaksi where id = '".$dt_histori['id_transaksi']."'");
